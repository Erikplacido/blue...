...
{
    /**
     * FONTE ÚNICA DE VERDADE - PREÇOS BASE DINÂMICOS
     * IMPORTANTE: Valores serão carregados do banco de dados
     */
    private static $servicePrices = [
        // Valores serão carregados dinamicamente - não usar fallbacks fixos
    ];
    ...
    private static function getServicePrice($serviceId) 
    {
        ...
        error_log("❌ PricingEngine: Service ID $serviceId not found - MUST load from database");
        
        // Tentar carregar do banco de dados
        $databasePrice = self::loadPriceFromDatabase($serviceId);
        if ($databasePrice > 0) {
            return $databasePrice;
        }
        
        // Se falhar completamente, erro crítico
    ...
    /**
     * Carregar preço do banco de dados
     */
    private static function loadPriceFromDatabase($serviceId) {
        try {
            ...
            $stmt = $connection->prepare("SELECT base_price FROM services WHERE id = ? AND is_active = 1");
            $stmt->execute([$serviceId]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if ($result && $result['base_price'] > 0) {
                error_log("✅ PricingEngine: Loaded price from database for service $serviceId: " . $result['base_price']);
                return (float)$result['base_price'];
            }
            
            error_log("❌ PricingEngine: No price found in database for service ID: $serviceId");
            return 0;
            
        } catch (Exception $e) {
            error_log("❌ PricingEngine Database Error: " . $e->getMessage());
            return 0;
        }
    }
}
